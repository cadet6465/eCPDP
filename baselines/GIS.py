        ...
        for i in range(self.iteration):
            ...
            for testPart in TestParts:
                ...
                try:
                    ...
                    pred = self.model.predict(testPart[:, :-1])
                    prediction = np.concatenate((prediction,pred))
                    prob = self.model.predict_proba(testPart[:, :-1])
                    probability = np.concatenate((probability,pred))

                except :
                    prediction = np.concatenate((prediction, np.random.randint(0, 2, testPart.shape[0])))
                    probability = np.concatenate((probability, np.random.rand(0.4, 0.6, testPart.shape[0])))
